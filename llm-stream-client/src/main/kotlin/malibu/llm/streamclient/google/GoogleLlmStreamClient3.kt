...
import malibu.llm.streamclient.LlmStreamInputItem
...
import malibu.llm.streamclient.exception.UnsupportedReasoningEffortTypeException
import malibu.llm.streamclient.exception.UnsupportedSummaryTypeException
    ...
    private fun mapToolChoice(choice: ResponseCreateParams.ToolChoice?): GoogleInteractionToolChoice? {
        ...
        return choice.accept(
            object : ResponseCreateParams.ToolChoice.Visitor<GoogleInteractionToolChoice?> {
                override fun visitOptions(options: ToolChoiceOptions): GoogleInteractionToolChoice {
                    return when (options.value()) {
                        ToolChoiceOptions.Value.NONE -> GoogleInteractionToolChoice.Type("none")
                        ToolChoiceOptions.Value.AUTO -> GoogleInteractionToolChoice.Type("auto")
                        ToolChoiceOptions.Value.REQUIRED -> GoogleInteractionToolChoice.Type("any")
                        ToolChoiceOptions.Value._UNKNOWN -> GoogleInteractionToolChoice.Type("auto")
                    }
                }

                override fun visitAllowed(allowed: ToolChoiceAllowed): GoogleInteractionToolChoice {
                    val mode = when (allowed.mode().value()) {
                        ToolChoiceAllowed.Mode.Value.AUTO -> "auto"
                        ToolChoiceAllowed.Mode.Value.REQUIRED -> "any"
                        ToolChoiceAllowed.Mode.Value._UNKNOWN -> "auto"
                    }
                    return GoogleInteractionToolChoice.AllowedTools(mode = mode)
                }

                override fun visitTypes(types: ToolChoiceTypes): GoogleInteractionToolChoice {
                    return GoogleInteractionToolChoice.Type("any")
                }

                override fun visitFunction(function: ToolChoiceFunction): GoogleInteractionToolChoice {
                ...
                override fun visitMcp(mcp: ToolChoiceMcp): GoogleInteractionToolChoice {
                    return GoogleInteractionToolChoice.Type("any")
                }

                override fun visitCustom(custom: ToolChoiceCustom): GoogleInteractionToolChoice {
                    return GoogleInteractionToolChoice.Type("any")
                }

                override fun visitApplyPatch(applyPatch: ToolChoiceApplyPatch): GoogleInteractionToolChoice {
                    return GoogleInteractionToolChoice.Type("any")
                }

                override fun visitShell(shell: ToolChoiceShell): GoogleInteractionToolChoice {
                    return GoogleInteractionToolChoice.Type("any")
                }
            }
        )
